...
function InjectKBOSCTab() {
    ...
    scr.textContent = "(" + function () {
        ...
        // Polling function to wait until the C# globalized UI object is ready
        function initializeWhenReady() {
            const Ui = window.Ui || window._Ui;
            
            // If it's not ready yet, wait 30ms and try again
            if (!Ui || !Ui.ComponentType) {
                setTimeout(initializeWhenReady, 30);
                return;
            }
            ...
            const SECTIONS = [
                ...
                },
                {
                    name: 'Track Space HMD Smooth', priority: 9, settings: [
                        ...
                        { type: Ui.ComponentType.Toggle, id: 'XSOverlayTweak.TrackSpaceHMDLockRoll', name: 'Lock Rolling', description: 'Prevents the HMD-attached Capture Overlay from rolling with HMD.', default: true },
        ...
        // Kick off the initial check loop
        initializeWhenReady();

    }.toString() + ")();"; // Removed the outer evaluation param since it polls internally now
